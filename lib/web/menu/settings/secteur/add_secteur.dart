...
class AddSecteurDialog extends StatefulWidget {
  const AddSecteurDialog({super.key});


  @override
  State<AddSecteurDialog > createState() => _AddSecteurDialogState();
}

class _AddSecteurDialogState extends State<AddSecteurDialog> {
  ...
  _submit() async {
    ...
    bool res = await SecteurController().addASecteur(secteurController.text, idArrond) ;
    ...
    if(res){
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Secteur Ajouté') , backgroundColor: vert(),),
      );
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Center(child: CustomText('Ajouter Secteur ', color: vert(),fontWeight: FontWeight.w700,tex: 1.5,)),
